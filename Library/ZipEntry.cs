...
namespace Ionic.Utils.Zip
{
    ...
    public class ZipEntry
    {
        ...
        private static bool ReadHeader(ZipEntry ze, System.Text.Encoding defaultEncoding)
        {
            ...
            int signature = Ionic.Utils.Zip.SharedUtilities.ReadSignature(ze._s);
            bytesRead += 4;
            ...
            int timeBlob = block[i++] + block[i++] * 256 + block[i++] * 256 * 256 + block[i++] * 256 * 256 * 256;
            // transform the time data into something usable (a DateTime)
            ze._LastModified = Ionic.Utils.Zip.SharedUtilities.PackedToDateTime(timeBlob);
            ...
            if ((ze._BitField & 0x0008) == 0x0008)
            {
                ...
                long SizeOfDataRead = Ionic.Utils.Zip.SharedUtilities.FindSignature(ze._s, ZipConstants.ZipEntryDataDescriptorSignature);
        ...
        private void WriteHeader(System.IO.Stream s, byte[] bytes)
        {
            ...
            // LastMod
            int timeBlob = Ionic.Utils.Zip.SharedUtilities.DateTimeToPacked(LastModified);

            bytes[i++] = (byte)(timeBlob & 0x000000FF);
            bytes[i++] = (byte)((timeBlob & 0x0000FF00) >> 8);
