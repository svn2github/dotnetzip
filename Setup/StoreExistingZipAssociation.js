...
function DisplayMessageBox(message, options) {
    if (options == null) {
        options = msiMessageLevel.User + msiButtonType.Ok + msiDefaultButton.Default1;
    }
...
function DisplayDiagnostic(message) {
    ...
    var options = msiMessageLevel.User + msiButtonType.Ok + msiDefaultButton.Default1;
    var record = Session.Installer.CreateRecord(1);
    record.StringData(0) = "[1]";
    record.StringData(1) = message;
...
function PreserveFileAssociation() {
    ...
    var regValue1 = "HKEY_CURRENT_USER\\SOFTWARE\\Dino Chiesa\\DotNetZip Tools v1.9\\PriorZipAssociation";
